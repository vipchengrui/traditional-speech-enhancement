...
# input wave file 
f = wave.open('in_TIMIT_1_TEST-White+5db.wav')
...
for n in range(0, Nframes):
    ...
    if n == 0:
        para = IN.Init_Weight(sig2,fs).info()    
    else:
        para = EN.Est_Weight(sig2,para).est()
...
# save wave file
wf = wave.open('out_TIMIT_1_TEST-White+5db.wav', 'wb')
...
# plot wave
plt.figure(1)
plt.suptitle('Log-MMSE_SPU based on Noise Estimation ( Weight , TIMIT_1_TEST-White+5db )')
plt.subplot(311)
plt.plot(x1)
